...
public class ProductCard {
	...
	private static final By REVIEWS_COUNT = By
			.xpath("//span[@data-visual-selector-id='components/gift/ProductCard76']");
	...
	public String getDescription() {
		return find(DESCRIPTION).getText().trim();
	}

	public String getPriceText() {
		return find(PRICE).getText().trim();
	}

	public String getRatingText() {
		return find(RATING_BLOCK).getText().trim();
	}

	public double getPrice() {
		return parsePrice(getPriceText());
	}
	...
	public String getThumbnailUrl() {
		return find(THUMBNAIL_IMG).getAttribute("src");
	}

	// --------- Images ----------
	public void toggleImages() {
		find(IMAGES_BUTTON).click();
	}

	public WebElement getImageBtn() {
		return find(IMAGES_BUTTON);
	}
	...
	public boolean isImagesOpen() {
		...
		return !root.findElements(By.cssSelector("[data-testid^='product-additional-image-" + getCardIndex() + "-']"))
				.isEmpty();
	}

	public void waitImagesOpen() {
		wait.until(d -> isImagesOpen());
	}

	public void waitImagesClosed() {
		wait.until(d -> !isImagesOpen());
	}
	...
	public void toggleReviews() {
		find(REVIEWS_BUTTON).click();
	}

	public WebElement getReviewBtn() {
		return find(REVIEWS_BUTTON);
	}
	...
	public void waitReviewsOpen() {
		wait.until(d -> isReviewsOpen());
	}

	public void waitReviewsClosed() {
		wait.until(d -> !isReviewsOpen());
	}
	...
	public int getReviewCount() {
		WebElement reviewsElement = find(REVIEWS_COUNT);
		String fullText = reviewsElement.getText();
		String reviewsCount = fullText.replaceAll("[^0-9]", "");
	...
	private ReviewSnapshot parseReview(WebElement reviewRoot) {
		...
		if (!spans.isEmpty())
			author = spans.get(0).getText().trim();
		...
		if (!ps.isEmpty())
			text = ps.get(0).getText().trim();
		if (ps.size() >= 2)
			date = ps.get(1).getText().trim();
	...
	private int getCardIndex() {
		...
		if (testId == null)
			throw new IllegalStateException("Card root is missing data-testid");
		Matcher m = Pattern.compile("product-card-(\\d+)").matcher(testId);
		if (!m.find())
			throw new IllegalStateException("Unexpected card data-testid: " + testId);
	...
	private static double parsePrice(String priceText) {
		...
		if (cleaned.isBlank())
			throw new IllegalArgumentException("Cannot parse price: " + priceText);
